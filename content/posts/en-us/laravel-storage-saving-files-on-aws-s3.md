...
I am going to use the same code in order to show how to save files in AWS S3, Amazon's storage
service.
...
In your AWS user menu, go to My Security Credentials, create a new Access Key and copy the keys that
are generated.

**Setup**

Laravel already comes with the S3 configuration variables by default in the `.env` file, just add
your account and
bucket data.
...
You need to install a [package](https://github.com/thephpleague/flysystem-aws-s3-v3) in order for
...
I added a new route for the method where I will implement the upload the files by integrating with
the S3 service.
...
The second parameter of the `store` method is the `disk` where we will save the file. We pass `s3`
into it and Laravel
...
There are several ways to access files on S3: through download, temporary addresses, etc. This also
varies depending
on the access permissions that are configured on the bucket.

Below, I share an example of a route where I pass the file name that I want to access and, in the
controller, I
